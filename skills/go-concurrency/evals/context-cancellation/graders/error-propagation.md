...
The agent should advise checking the error from each service call and returning early on failure rather than continuing to invoke the remaining services. Calling a service with an already-expired context wastes nothing but a function call overhead, but it provides no useful result and the error check is still required. Explicit early returns make intent clear and prevent unnecessary work:
